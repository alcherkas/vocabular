...
## Seeder EN-16 — vocab/seeder-en-16

**Date:** 2025-07-28
**Agent:** Vocabulary Seeder (EN)
**Branch:** vocab/seeder-en-16
**Worktree:** /Users/aleksandrcherkas/Documents/GitHub/vocabular-wt-seeder-en-16

### What was done
Added 100 new English C1/C2 vocabulary stubs to `words_staging.json`. Terms span academic, literary, philosophical, legal, and professional domains (e.g., adumbrate, apostasy, asceticism, atavism, calumny, codicil, defalcation, dilettante, dissemble, erudition, exculpate, expropriate, forensic, fractious).

### What went well
- Zero duplicates against both staging and production on first pass.
- Validator passed cleanly after fixing `lang` → `language` field name (the task spec used `lang`, but the schema requires `language`).

### What went wrong / surprises
- Task instructions specified `"lang": "en"` but the actual schema field is `"language": "en"`. Required a fix pass after first validation failure.

### Lessons learned
- Always check an existing stub entry structure before generating new ones, or run validation early.
